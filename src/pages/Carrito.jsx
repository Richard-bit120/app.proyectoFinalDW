...
import { Container, Typography, Box, Button, Grid, Paper, IconButton, Divider, Alert } from '@mui/material';
...
export const Carrito = () => {
  ...
  const { cart, total, eliminarProducto, vaciarCarrito } = useContext(CartContext);
    ...
    return (
      <Container maxWidth="md" sx={{ mt: 4 }}>
        <Alert severity="info" sx={{ mb: 3 }}>
          Tu carrito de compras está vacío actualmente. ¡Anímate a agregar algún producto!
        </Alert>
        <Button component={RouterLink} to="/productos" variant="contained" startIcon={<ArrowBackIcon />}>
        ...
        <Grid item xs={12} md={8}>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            {cart.map((item) => {
              ...
              const imageUrl = item.product.images[0]?.replace(/[\[\]"]/g, '') || 'https://via.placeholder.com/150';
              ...
              return (
                <Paper key={item.product.id} elevation={1} sx={{ p: 2, display: 'flex', alignItems: 'center', gap: 2 }}>
                  {/* Requisito: Imagen */}
                  <Box
                    component="img"
                    src={imageUrl}
        ...
        <Grid item xs={12} md={4}>
          <Paper elevation={3} sx={{ p: 3, borderRadius: 2, bgcolor: 'background.paper' }}>
            ...
            <Button
              ...
              onClick={() => alert('¡Simulación de checkout completada con éxito!')}
